...
private[metrics] class DefaultRate private[metrics] (val address: MetricAddress,
                                                     val pruneEmpty: Boolean,
                                                     intervals: Seq[FiniteDuration])
    extends Rate {

  private val maps: Map[FiniteDuration, CollectionMap[TagMap]] = intervals.map { i =>
    (i, new CollectionMap[TagMap])
  }.toMap
...
object Rate {
  ...
  /**
    ...
    * @return
    */
  def apply(address: MetricAddress, pruneEmpty: Boolean = false, enabled: Boolean = true)(
      implicit ns: MetricNamespace): Rate = {
    ns.getOrAdd(address) { (fullAddress, config) =>
      createRate(fullAddress, pruneEmpty, enabled, config.intervals)
    }
  }

  private def createRate(address: MetricAddress,
                         pruneEmpty: Boolean,
                         enabled: Boolean,
                         intervals: Seq[FiniteDuration]): Rate = {
    if (enabled) {
