...
abstract class MetricIntegrationSpec(_system: ActorSystem)
...
class EventuallyEqualMatcher[T](expected: T, tries: Int, timeout: FiniteDuration, retryInterval: FiniteDuration)
    extends Matcher[() => Future[T]] {
...
trait EventuallyEquals {
  ...
  def eventuallyEqual[T](expected: T,
                         tries: Int = 5,
                         timeout: FiniteDuration = 500.milliseconds,
                         retryInterval: FiniteDuration = 100.milliseconds) =
    new EventuallyEqualMatcher[T](expected, tries, timeout, retryInterval)
