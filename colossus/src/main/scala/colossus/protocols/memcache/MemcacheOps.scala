...
trait MemcacheClient[M[_]] extends LiftedClient[Memcache, M] {
  ...
  protected def executeCommand[T](c: MemcacheCommand, key: ByteString)(
      goodCase: PartialFunction[MemcacheReply, M[T]]): M[T] = {
    executeAndMap(c) {
      goodCase orElse {
        ...
        case x =>
          failure(
            UnexpectedMemcacheReplyException(
              s"Unexpected response $x when executing ${c.commandName.utf8String} on ${key.utf8String}"))
      }
    }
  }
...
object MemcacheClient {
  ...
  implicit object MemcacheClientLifter extends ClientLifter[Memcache, MemcacheClient] {
    ...
    override def lift[M[_]](client: Sender[Memcache, M], clientConfig: Option[ClientConfig])(
        implicit async: Async[M]): MemcacheClient[M] = {
