...
trait RedisClient[M[_]] extends LiftedClient[Redis, M] {
  ...
  protected def executeCommand[T](c: Command, key: ByteString)(goodCase: PartialFunction[Reply, M[T]]): M[T] = {
    executeAndMap(c) {
      goodCase orElse {
        case ErrorReply(msg) =>
          failure(RedisException(s"Received error msg when executing ${c.command} on key ${key.utf8String} : $msg"))
        case x =>
          failure(
            UnexpectedRedisReplyException(s"Unexpected response $x when executing ${c.command} on ${key.utf8String}"))
      }
    }
  }
  ...
  protected def mBulkReplyCommand(c: Command, key: ByteString): M[Seq[ByteString]] = executeCommand(c, key) {
    case MBulkReply(x) => {
      ...
      if (b.length == x.length) {
        ...
        failure(UnexpectedRedisReplyException(
          s"Unexpected Reply type found in MBulkReply, expected all BulkReplies when executing ${c.command} on ${key.utf8String}"))
      }
    }
    case EmptyMBulkReply => success(Nil)
  }

  protected def mBulkReplyOptionCommand(c: Command, key: ByteString): M[Seq[Option[ByteString]]] =
    executeCommand(c, key) {
      case MBulkReply(x) => {
        val b = x.collect {
        ...
        if (b.length == x.length) {
          ...
          failure(UnexpectedRedisReplyException(
            s"Unexpected Reply type found in MBulkReply, expected all BulkReply or NilReply when executing ${c.command} on ${key.utf8String}"))
        }
      }
    }
  ...
  def expire(key: ByteString, ttl: FiniteDuration): M[Boolean] =
    integerReplyBoolCommand(cmd(CMD_EXPIRE, key, seconds(ttl)), key)
  ...
  def hincrby(key: ByteString, field: ByteString, amount: Long): M[Long] =
    integerReplyCommand(cmd(CMD_HINCRBY, key, field, amount), key)
  ...
  def hmget(key: ByteString, fields: ByteString*): M[Seq[Option[ByteString]]] =
    mBulkReplyOptionCommand(cmd(CMD_HMGET, key +: fields: _*), key)

  def hmset(key: ByteString, fields: ByteString*): M[Boolean] =
    stringReplyCommand(cmd(CMD_HMSET, key +: fields: _*), key)

  def hset(key: ByteString, field: ByteString, value: ByteString): M[Boolean] =
    integerReplyBoolCommand(cmd(CMD_HSET, key, field, value), key)

  def hsetnx(key: ByteString, field: ByteString, value: ByteString): M[Boolean] =
    integerReplyBoolCommand(cmd(CMD_HSETNX, key, field, value), key)
  ...
  def incrbyfloat(key: ByteString, amount: Double): M[Double] =
    bulkReplyAsDoubleCommand(cmd(CMD_INCRBYFLOAT, key, amount), key)
  ...
  def lpush(key: ByteString, values: ByteString*): M[Long] =
    integerReplyCommand(cmd(CMD_LPUSH, key +: values: _*), key)

  def lpushx(key: ByteString, value: ByteString): M[Long] = integerReplyCommand(cmd(CMD_LPUSHX, key, value), key)

  def lrange(key: ByteString, start: Long, end: Long): M[Seq[ByteString]] =
    mBulkReplyCommand(cmd(CMD_LRANGE, key, start, end), key)

  def lrem(key: ByteString, count: Long, value: ByteString): M[Long] =
    integerReplyCommand(cmd(CMD_LREM, key, count, value), key)

  def lset(key: ByteString, index: Long, value: ByteString): M[Boolean] =
    stringReplyCommand(cmd(CMD_LSET, key, index, value), key)

  def ltrim(key: ByteString, start: Long, end: Long): M[Boolean] =
    stringReplyCommand(cmd(CMD_LTRIM, key, start, end), key)

  def mget(keys: ByteString*): M[Seq[Option[ByteString]]] =
    mBulkReplyOptionCommand(cmd(CMD_MGET, keys: _*), multiKey(keys))

  def mset(keysAndValues: ByteString*): M[Boolean] =
    stringReplyCommand(cmd(CMD_MSET, keysAndValues: _*), multiKey(keysAndValues))

  def msetnx(keysAndValues: ByteString*): M[Boolean] =
    integerReplyBoolCommand(cmd(CMD_MSETNX, keysAndValues: _*), multiKey(keysAndValues))

  def persist(key: ByteString): M[Boolean] = integerReplyBoolCommand(cmd(CMD_PERSIST, key), key)

  def pexpire(key: ByteString, ttl: FiniteDuration): M[Boolean] =
    integerReplyBoolCommand(cmd(CMD_PEXPIRE, key, millis(ttl)), key)

  def pexpireat(key: ByteString, unixTimeInMillis: Long): M[Boolean] =
    integerReplyBoolCommand(cmd(CMD_PEXPIREAT, key, unixTimeInMillis), key)

  def psetex(key: ByteString, value: ByteString, ttl: FiniteDuration): M[Boolean] =
    stringReplyCommand(cmd(CMD_PSETEX, key, millis(ttl), value), key)
  ...
  def rename(source: ByteString, destination: ByteString): M[Boolean] =
    stringReplyCommand(cmd(CMD_RENAME, source, destination), source)
  ...
  def rpush(key: ByteString, values: ByteString*): M[Long] =
    integerReplyCommand(cmd(CMD_RPUSH, key +: values: _*), key)
  ...
  def set(key: ByteString,
          value: ByteString,
          notExists: Boolean = false,
          exists: Boolean = false,
          ttl: Duration = Duration.Inf): M[Boolean] = {
  ...
  def sinter(key: ByteString, keys: ByteString*): M[Set[ByteString]] =
    map(mBulkReplyCommand(cmd(CMD_SINTER, key +: keys: _*), key))(_.toSet)
  ...
  def sismember(key: ByteString, value: ByteString): M[Boolean] =
    integerReplyBoolCommand(cmd(CMD_SISMEMBER, key, value), key)
  ...
  def srandmember(key: ByteString, count: Long): M[Seq[ByteString]] =
    mBulkReplyCommand(cmd(CMD_SRANDMEMBER, key, ByteString(count.toString)), key)
  ...
  def sunion(key: ByteString, keys: ByteString*): M[Set[ByteString]] =
    map(mBulkReplyCommand(cmd(CMD_SUNION, key +: keys: _*), key))(_.toSet)
  ...
  def zadd(key: ByteString, valuesAndArguments: ByteString*): M[Double] =
    executeCommand(cmd(CMD_ZADD, key +: valuesAndArguments: _*), key) {
      case IntegerReply(x) => success(x)
      case BulkReply(x)    => success(x.utf8String.toDouble)
    }
  ...
  def zcount(key: ByteString, from: Option[Double] = None, to: Option[Double]): M[Long] = {
    val fromB = from.fold(ByteString("-inf")) { x =>
      ByteString(x.toString)
    }
    val toB = to.fold(ByteString("+inf")) { x =>
      ByteString(x.toString)
    }
    integerReplyCommand(cmd(CMD_ZCOUNT, key, fromB, toB), key)
  }

  def zincrby(key: ByteString, amount: Double, value: ByteString): M[Double] =
    bulkReplyAsDoubleCommand(cmd(CMD_ZINCRBY, key, bs(amount), value), key)
  ...
  def zremrangebylex(key: ByteString, min: ByteString, max: ByteString): M[Long] =
    integerReplyCommand(cmd(CMD_ZREMRANGEBYLEX, key, min, max), key)

  def zremrangebyrank(key: ByteString, start: Long, stop: Long): M[Long] =
    integerReplyCommand(cmd(CMD_ZREMRANGEBYRANK, key, bs(start), bs(stop)), key)

  def zremrangebyscore(key: ByteString, min: ByteString, max: ByteString): M[Long] =
    integerReplyCommand(cmd(CMD_ZREMRANGEBYSCORE, key, min, max), key)
...
object RedisClient {
  ...
  implicit object RedisClientLifter extends ClientLifter[Redis, RedisClient] {
    ...
    override def lift[M[_]](client: Sender[Redis, M], clientConfig: Option[ClientConfig])(
        implicit async: Async[M]): RedisClient[M] = {
