...
object Combinators {
  ...
  class FlatMapParser[A, B](orig: Parser[A], f: A => Parser[B]) extends Parser[B] {
    ...
    def parse(data: DataBuffer) = {
      if (mapped.isDefined) {
        mapped.get.parse(data).map { x =>
          mapped = None; x
        }
      } else {
        orig.parse(data).map { r =>
          mapped = Some(f(r))
        }
        if (mapped.isDefined) {
          mapped.get.parse(data).map { x =>
  ...
  implicit class ByteArrayOps(val parser: Parser[Array[Byte]]) extends AnyVal {
    ...
    def asByteString: Parser[ByteString] = parser >> { d =>
      ByteString(d)
    }
  }
  ...
  /**
    ...
    */
  def bytes(num: Parser[Int], maxSize: DataSize, maxInitBufferSize: DataSize): Parser[Array[Byte]] = num |> { s =>
    bytes(s, maxSize, maxInitBufferSize)
  }
  ...
  /**
    ...
    * the returned value
    */
  def bytesUntil(terminus: Array[Byte],
                 includeTerminusInData: Boolean = false,
                 sizeHint: Int = 32): Parser[Array[Byte]] = new Parser[Array[Byte]] {
  ...
  def short: Parser[Short] = bytes(2) >> { b =>
    ByteBuffer.wrap(b).getShort
  }

  def int: Parser[Int] = bytes(4) >> { b =>
    ByteBuffer.wrap(b).getInt
  }

  def long: Parser[Long] = bytes(8) >> { b =>
  ...
  /** Parse a string until a designated byte is encountered
    ...
    *
    */
  def stringUntil(terminus: Byte,
                  toLower: Boolean = false,
                  minSize: Option[Int] = None,
                  allowWhiteSpace: Boolean = true,
                  ltrim: Boolean = false): Parser[String] = new Parser[String] {
  ...
  /**
    ...
    */
  class RepeatZeroParser[T: scala.reflect.ClassTag](parser: Parser[T])(implicit zero: Zero[T])
      extends Parser[Array[T]] {
  ...
  class FoldZeroParser[T, U](parser: Parser[T], init: => U)(folder: (T, U) => U)(implicit zero: Zero[T])
      extends Parser[U] {
  ...
  def foldZero[T, U](parser: Parser[T], init: => U)(folder: (T, U) => U)(implicit zero: Zero[T]) =
    new FoldZeroParser(parser, init)(folder)
  ...
  def line[T](constructor: Array[Byte] => T, includeNewLine: Boolean): Parser[T] =
    new LineParser(constructor, includeNewLine)
