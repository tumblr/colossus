...
case class ClientConfig(
    ...
    loadBalancerSettings: LoadBalancerSettings = LoadBalancerSettings(
      BackoffPolicy(0.milliseconds, BackoffMultiplier.Linear(0.seconds)),
      UniqueHosts(None)
    )
)
...
object ClientConfig {
  ...
  def load(config: Config): ClientConfig = {
    ...
    val loadBalancerSettings = LoadBalancerSettings(config.getConfig("load-balancer"))
...
class ServiceClient[P <: Protocol](
  ...
  val controllerConfig =
    ControllerConfig(config.pendingBufferSize, metricsEnabled = true, inputMaxSize = config.maxResponseSize)
  ...
  override def onBind() {
    if (clientState == ClientState.Initializing) {
      ...
      throw new StaleClientException(
        "This client has already been manually disconnected and cannot be reused, create a new one.")
    }
  }
  ...
  private def sendNow(request: Request)(handler: ResponseHandler) {
    ...
    if (canSend) {
      pending.push(sourced) match {
        ...
        case PushResult.Full(trig) =>
          failRequest(sourced, new ClientOverloadedException(s"Error sending ${request}: Client is overloaded"))
  ...
  private def purgeBuffers(reason: Throwable) {
    sentBuffer.filterScan { s =>
      failRequest(s, reason); true
    }
    if (failFast) {
      pending.filterScan { s =>
        failRequest(s, reason); true
      }
    }
  }
