...
object IOSystem {
  ...
  def apply(config: IOSystemConfig, metrics: MetricSystem)(implicit system: ActorSystem): IOSystem = {
    new IOSystem(config, metrics, system)
  }
 ...
 * loops.
 */
class IOSystem private[colossus](val config: IOSystemConfig, val  metrics: MetricSystem, val actorSystem: ActorSystem) {
  import IOCommand._
  ...
  private[colossus] val workerManager = actorSystem.actorOf(Props(
    classOf[WorkerManager], 
    WorkerManagerConfig(config.numWorkers, metrics),
    workers,
    this  
  ), name = s"${config.name}-manager")
  workerManager ! WorkerManager.Initialize
...
object IOCommand {
   ...
   */
  case class BindWithContext(context: Context, item: Context => WorkerItem)
