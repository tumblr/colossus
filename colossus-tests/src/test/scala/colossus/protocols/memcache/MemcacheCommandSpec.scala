...
class MemcacheCommandSuite extends FlatSpec with ShouldMatchers{
  ...
  it should "format a SET correctly" in {
    val experimental = Set(MemcachedKey("key"), ByteString("value"), 30) // key, value, ttl
  ...
  it should "format an ADD correctly" in {
    val experimental = Add(MemcachedKey("key"), ByteString("magic"), 30)
  ...
  it should "format a REPLACE correctly" in {
    val experimental = Replace(MemcachedKey("key"), ByteString("magic"), 30)
  ...
  it should "format an APPEND correctly" in {
    val experimental = Append(MemcachedKey("key"), ByteString("magic"))
  ...
  it should "format a PREPEND correctly" in {
    val experimental = Prepend(MemcachedKey("key"), ByteString("magic"))
...
/*  it should "format a CAS correctly" in {
  ...
  it should "format DELETE correctly" in {
    val experimental = Delete(MemcachedKey("key"))
  ...
  it should "format INCR correctly" in {
    val experimental = Incr(MemcachedKey("key"), 1L)
  ...
  it should "format DECR correctly" in {
    val experimental = Decr(MemcachedKey("key"), 1L)
  ...
  it should "format TOUCH correctly" in {
    val experimental = Touch(MemcachedKey("key"), 30)
