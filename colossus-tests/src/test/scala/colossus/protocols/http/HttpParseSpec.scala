...
class HttpParserSuite extends WordSpec with MustMatchers {
  ...
  "http request parser" must {
    ...
    "parse request with no headers" in {
      ...
      val parser = requestParser
      val expected = HttpRequest(
        HttpRequestHead(method = HttpMethod.Get, url = "/hello", version = HttpVersion.`1.1`, headers = Nil),
        HttpBody.NoBody)
      parser.parse(DataBuffer(ByteString(req))) must equal(Some(expected))
    }
    ...
    "parse a generated request" in {
      val req =
        HttpRequest(HttpRequestHead(version = HttpVersion.`1.1`, method = HttpMethod.Get, url = "/foo", headers = Nil),
                    HttpBody.NoBody)
      val d = new DynamicOutBuffer(100, false)
      req.encode(d)
      val data   = d.data
