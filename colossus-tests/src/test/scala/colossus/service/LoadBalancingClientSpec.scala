...
class LoadBalancingClientSpec extends ColossusSpec with MockFactory {
  ...
  def mockClient(port: Int, customReturn: Option[Try[Int]] = None): C =
    mockClient(new InetSocketAddress("0.0.0.0", port), customReturn)
  ...
  def addrs(num: Int) = (1 to num).map { i =>
    new InetSocketAddress("0.0.0.0", i)
  }
  ...
  "LoadBalancingClient" must {
    ...
    "evenly divide requests among clients" in {
      ...
      (1 to 5).foreach { num =>
        ...
        val clients = addresses.map { a =>
          fakeSender()
        }
        val (probe, worker) = FakeIOSystem.fakeWorkerRef
        val l =
          new LoadBalancingClient[PR](worker, staticClients(clients.toList), maxTries = 2, initialClients = addresses)
        (1 to ops).foreach { i =>
          l.send("hey").execute()
        }
        clients.foreach { c =>
    ...
    "properly failover requests" in {
      ...
      val (probe, worker) = FakeIOSystem.fakeWorkerRef
      val l =
        new LoadBalancingClient[PR](worker, staticClients(List(good, bad)), maxTries = 2, initialClients = addrs(2))
    ...
    "open multiple connections to a single host" in {
      val hosts = (1 to 3).map { i =>
        new InetSocketAddress("0.0.0.0", 1)
      }
      val clients = (1 to 3).map { i =>
        mockClient(1, Some(Success(i)))
      }
      val (probe, worker) = FakeIOSystem.fakeWorkerRef
    ...
    "removing by address removes all connections" in {
      val hosts = (new InetSocketAddress("1.1.1.1", 5)) :: (1 to 2).map { i =>
        new InetSocketAddress("0.0.0.0", 1)
      }.toList
      val (probe, worker) = FakeIOSystem.fakeWorkerRef
    ...
    "update doesn't open duplicate connections when allowDuplicates is false" in {
      ...
      val l = new LoadBalancingClient[PR](worker,
                                          staticClients(clients.toList),
                                          maxTries = 2,
                                          initialClients = List(address))
      CallbackAwait.result(l.send("hey"), 1.second) mustBe 1
      CallbackAwait.result(l.send("hey"), 1.second) mustBe 1
